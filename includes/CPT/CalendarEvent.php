...
class CalendarEvent
{
    ...
    public static function eventFields()
    {
        ...
        $cmb_info->add_field([
            'id' => 'ics-warning',
            'type' => 'title',
            'description' => '<div class="alert alert-danger" style="padding: 1em;">' . sprintf(
                /* translators: 1: <strong>, 2: </strong>. */
                __('%1$sThis event is generated by feed import.%2$s Changes will be overwritten by the next sync.', 'rrze-calendar'),
                '<strong>',
                '</strong>'
            ) . '</div>',
        ...
        $cmb_info->add_field([
            ...
            'attributes'  => [
                'data-datepicker' => wp_json_encode([
                    ...
                    'dayNames'        => Utils::daysOfWeek('long'),
                    'dayNamesShort'   => Utils::daysOfWeek('short'),
                    'dayNamesMin'     => Utils::daysOfWeek('short'),
...
?>
    ...
    public static function displayEventMain($data)
    {
        ...
        echo wpautop(do_shortcode(make_clickable($data['description'])));
        echo '</div>';
