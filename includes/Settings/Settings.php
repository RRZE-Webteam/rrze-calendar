...
class Settings
{
    ...
    public function styling()
    {
        ...
        echo '<style>.rrze-calendar-settings-error {color: #d63638; margin: 5px 0;}</style>';
    }
    ...
    {
        if (empty($this->tabs)) {
            $tab = $this->addTab(__('Unnamed tab', 'rrze-calendar'));
        } else {
            $tab = end($this->tabs);
        }
    ...
    {
        if (
            !isset($_POST['rrze_calendar_settings_save'])
            || !wp_verify_nonce(
                $_POST['rrze_calendar_settings_save'],
                'rrze_calendar_settings_save_' . $this->optionName
            )
        ) {
            return;
        }
        ...
        if (!current_user_can($this->capability)) {
            wp_die(__('You do not have enough permissions to do that.', 'rrze-calendar'));
        }

        $currentOptions = $this->getOptions();
        $submittedOptions = apply_filters('rrze_calendar_settings_new_options', $_POST[$this->optionName] ?? [], $currentOptions);
        ...
        foreach ($this->getActiveTab()->getActiveSections() as $section) {
            foreach ($section->options as $option) {
                ...
                $value = apply_filters('rrze_calendar_settings_new_option_' . $option->implementation->getName(), $option->sanitize($value), $option->implementation);
        ...
        $this->flash->set('success', __('Settings saved.', 'rrze-calendar'));
    }
    ...
    public function updateOptions($options)
    {
        ...
        do_action('rrze_calendar_settings_after_update_option', $this->optionName, $options);
    }
}
